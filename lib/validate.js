...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'd1' ) ) {
		...
		if ( !isPositive( opts.d1 ) ) {
			return new TypeError( 'quantile()::invalid option. `d1` parameter must be a positive number. Option: `' + opts.d1 + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'd2' ) ) {
		opts.d2 = options.d2;
		if ( !isPositive( opts.d2 ) ) {
			return new TypeError( 'quantile()::invalid option. `d2` parameter must be a positive number. Option: `' + opts.d2 + '`.' );
		}
	}
